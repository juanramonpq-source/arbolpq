...
async function createSql(): Promise<Sql> {
  ...
  if (dbSource !== "neon" && isNetlifyRuntime()) {
    throw new Error(
      "Netlify without DATABASE_URL uses the shared document store, not PGLite.",
    );
  }
...
export function ensureDbReady(): Promise<void> {
  ...
  if (isNetlifyRuntime()) return Promise.resolve();
...
if (typeof window === "undefined" && dbSource === "pglite" && !isNetlifyRuntime()) {
