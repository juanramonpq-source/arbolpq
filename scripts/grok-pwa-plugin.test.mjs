...
test("uses the home-screen name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race" });
  assert.match(out, /apple-mobile-web-app-title" content="ArbolPQ"/);
});
...
test("renders the manifest with the family home-screen name", () => {
  const manifest = JSON.parse(renderWebManifest("wild-race.grok.me"));
  assert.equal(manifest.short_name, "ArbolPQ");
  assert.equal(manifest.name, "Árbol Pérez Quintanar");
